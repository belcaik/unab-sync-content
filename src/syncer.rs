...
use crate::fsutil::{atomic_rename, atomic_write, ensure_dir, sanitize_component, sanitize_filename_preserve_ext};
...
pub async fn run_sync(filter_course_id: Option<u64>, dry_run: bool, verbose: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ignored: std::collections::HashSet<String> = cfg.canvas.ignored_courses.iter().cloned().collect();
    ...
    let iter: Box<dyn Iterator<Item = crate::canvas::Course>> = if let Some(cid) = filter_course_id {
        ...
        let sel = courses.into_iter().filter(move |c| c.id == cid).collect::<Vec<_>>();
        if sel.is_empty() {
            tracing::warn!(course_id = cid, "course not found in active list; nothing to sync");
            return Ok(());
        }
        Box::new(sel.into_iter())
    } else {
        Box::new(courses.into_iter().filter(move |c| !ignored.contains(&c.id.to_string())))
    };
    ...
    for c in iter {
        ...
        let course_dir = PathBuf::from(&cfg.download_root).join(if code.is_empty() {
            ...
            format!("{}_{}", sanitize_component(&c.name), sanitize_component(code))
        });
        if !dry_run { ensure_dir(&course_dir).await?; }
        ...
        let assignments: std::collections::HashMap<u64, Assignment> = assignments_list
            .into_iter()
            .map(|a| (a.id, a))
            .collect();
        for m in modules {
            let (p, f) = sync_module(&cfg, &canvas, &httpctx, &course_dir, c.id, &assignments, &mut state, &m, dry_run, verbose).await?;
            total_pages += p;
            total_files += f;
        }

        if !dry_run { state.save(&state_path).await?; }
    }
    if dry_run {
        println!("DRY-RUN summary: pages to write: {}, files to download: {}", total_pages, total_files);
    }
    Ok(())
}
...
async fn sync_module(
    ...
    let module_dir = course_dir.join("Modules").join(format!("{}_{}",
        m.id,
        sanitize_component(&m.name)
    ));
    if !dry_run { ensure_dir(&module_dir).await?; }
        ...
        match item.kind.as_deref() {
            Some("Page") => {
                if let Some(page_url) = &item.page_url {
                    ...
                    let title = page.title.clone().unwrap_or_else(|| item.title.clone().unwrap_or_else(|| format!("item_{}", idx)));
                    ...
                    if state.get(&key).and_then(|s| s.content_hash.as_deref()) == Some(hash.as_str()) {
                        ...
                        if dry_run {
                            ...
                            state.set(key, ItemState { etag: None, updated_at: page.updated_at, size: Some(md.len() as u64), content_hash: Some(hash) });
                    ...
                    for fid in file_ids {
                        if !processed_ids.insert(fid) { continue; }
                        match canvas.get_file(fid).await {
                            Ok(f) => {
                                let fname = f.display_name.clone().or(f.filename.clone()).unwrap_or_else(|| format!("file_{}", fid));
                                let dest = module_dir.join("Attachments").join(sanitize_filename_preserve_ext(&fname));
                                let f_ext = dest.extension().and_then(|s| s.to_str()).unwrap_or_default();
            ...
            _ if item.html_url.as_deref().is_some_and(|u| is_course_page_url(u, course_id)) => {
                ...
                if let Some(slug) = extract_page_slug(item.html_url.as_ref().unwrap()) {
                    ...
                    let title = page.title.clone().unwrap_or_else(|| item.title.clone().unwrap_or_else(|| slug.clone()));
                    let html = page.body.unwrap_or_default();
                    let md = parse_html(&html);
                    let hash = sha1_hex(md.as_bytes());
                    if state.get(&key).and_then(|s| s.content_hash.as_deref()) == Some(hash.as_str()) {
                    ...
                    for fid in file_ids {
                        ...
                        match canvas.get_file(fid).await {
                            ...
                            Err(e) => warn!(course_id, module_id = m.id, file_id = fid, error = %e, "unable to fetch file (page link)"),
                        }
                    }
                }
            }
            ...
            Some("Assignment") => {
                if let Some(aid) = item.content_id {
                    if let Some(assign) = assignments.get(&aid) {
                        let atitle = assign.name.clone().unwrap_or_else(|| item.title.clone().unwrap_or_else(|| format!("assignment_{}", aid)));
                        let html = assign.description.clone().unwrap_or_default();
                        let md = parse_html(&html);
                        let key = format!("assignment:{}", aid);
                        let hash = sha1_hex(md.as_bytes());
                        let fname = format!("{:02}-ASSIGN-{}.md", idx + 1, sanitize_component(&atitle));
                        let dest = module_dir.join(fname);
                        if state.get(&key).and_then(|s| s.content_hash.as_deref()) == Some(hash.as_str()) {
                            if !dry_run && verbose { println!("SKIP assignment -> {}", dest.display()); }
                        } else if dry_run {
                            pages_planned += 1;
                            println!("DRY-RUN assignment -> {} ({} bytes)", dest.display(), md.len());
                        } else {
                            atomic_write(&dest, md.as_bytes()).await?;
                            state.set(key, ItemState { etag: None, updated_at: assign.updated_at.clone(), size: Some(md.len() as u64), content_hash: Some(hash) });
                        ...
                        for fid in file_ids {
                            ...
                            match canvas.get_file(fid).await {
                                ...
                                Err(e) => warn!(course_id, module_id = m.id, file_id = fid, error = %e, "unable to fetch file (assignment)"),
                            }
                        }
                    }
                }
...
async fn download_if_needed(
    ...
    let url = f.download_url.as_ref().or(f.url.as_ref()).ok_or("missing file url")?;

    // Probe HEAD for ETag/size
    let head = httpctx
        .send(httpctx.client.head(url))
        .await?;
    let status = head.status();
    if !status.is_success() {
        warn!(file_id = f.id, status = %status.as_u16(), "head non-success, will GET");
    }
    let etag = head.headers().get(header::ETAG).and_then(|h| h.to_str().ok()).map(|s| s.trim_matches('"').to_string());
    let size = head.headers().get(header::CONTENT_LENGTH).and_then(|h| h.to_str().ok()).and_then(|s| s.parse::<u64>().ok());
    ...
    let mut start = 0u64;
    if let Ok(meta) = tokio::fs::metadata(&part).await { start = meta.len(); }

    // GET with Range if resuming
    let mut req = httpctx.client.get(url);
    if start > 0 { req = req.header(header::RANGE, format!("bytes={}-", start)); }
    ...
    // Update state
    let final_size = match tokio::fs::metadata(dest).await { Ok(m) => Some(m.len()), Err(_) => size };
    state.set(key, ItemState { etag, updated_at: f.updated_at.clone(), size: final_size, content_hash: None });
    Ok(())
}
...
fn extract_page_slug(url: &str) -> Option<String> {
    let re = Regex::new(r"/courses/(\d+)/pages/([A-Za-z0-9_\-]+)").unwrap();
    re.captures(url).and_then(|c| c.get(2)).map(|m| m.as_str().to_string())
}
