...
pub fn sanitize_component<S: AsRef<str>>(s: S) -> String {
    ...
    if name.is_empty() { return "untitled".into(); }
    ...
    if final_s.len() > MAX_LEN { final_s[..MAX_LEN].to_string() } else { final_s }
}
...
pub fn sanitize_filename_preserve_ext<S: AsRef<str>>(s: S) -> String {
    ...
    if out.len() > MAX_LEN { out[..MAX_LEN].to_string() } else { out }
}
